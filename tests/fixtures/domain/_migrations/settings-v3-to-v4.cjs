...
async function migrate () {
  ...
  try {
    ...
    // Populate the Site.js cloud-init from the existing one.
    db.settings.apps[0].cloudInit = db.settings.vps.cloudInit

    // The cloudInit property is no longer used/required on the VPS settings.
    delete(db.settings.vps.cloudInit)
