...
export function parseSource(source) {
  let normalisedSource, nodeScript, script, moduleScript, style, svelteHead, svelteBody, svelteWindow, options

  normalisedSource = source
  ;({normalisedSource, extracted: nodeScript} = extract(normalisedSource, nodeScriptRegExp))
  ;({normalisedSource, extracted: script} = extract(normalisedSource, scriptRegExp))
  ;({normalisedSource, extracted: moduleScript} = extract(normalisedSource, moduleScriptRegExp))
  ;({normalisedSource, extracted: style} = extract(normalisedSource, styleRegExp))
  ;({normalisedSource, extracted: svelteHead} = extract(normalisedSource, svelteHeadRegExp))
  ;({normalisedSource, extracted: svelteBody} = extract(normalisedSource, svelteBodyRegExp))
  ;({normalisedSource, extracted: svelteWindow} = extract(normalisedSource, svelteWindowRegExp))
  ;({normalisedSource, extracted: svelteOptions} = extract(normalisedSource, svelteOptionsRegExp))

  // At this point, the only thing that remains in normalisedSource should be
  ...
  const componentClass = svelteHash(normalisedSource + script)
  
  normalisedSource = `${script ? `<script>${script}</script>` : ''}${moduleScript ? `\n\n<script context='module'>${moduleScript}</script>` : ''}${svelteHead ? `\n\n<svelte:head>${svelteHead}</svelte:head>` : ''}${svelteWindow ? `\n\n<svelte:window ${svelteWindow}/>` : ''}${svelteBody ? `\n\n<svelte:body ${svelteBody}/>` : ''}${svelteOptions ? `\n\n<svelte:options ${svelteOptions}/>` : ''}<div class='nodekit-${componentClass}' style='display: contents;'>${normalisedSource.trim()}</div>${style ? `<style>${style}</style>` : ''}`

  return { componentClass, normalisedSource, nodeScript, script, moduleScript, style, svelteHead, svelteBody, svelteWindow, svelteOptions }
}
