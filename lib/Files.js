...
export default class Files extends EventTarget {
  ...
  async initialise () {
    ...
    return new Promise((resolve, reject) => {
      // Note: we only watch for components if in development mode.
      const watcherGlob = `${this.pathToWatch}/**/*.@(page|${!process.env.PRODUCTION ? 'component|':''}socket|${HTTP_METHODS.join('|')})`
