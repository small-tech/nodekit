...
export default class Files {
  ...
  constructor (basePath = process.cwd()) {
    ...
    this.broadcastChannel.onmessage = event => {
      console.log('[Main process broadcast channel] Received', event.data)
    }
    ...
    // Open the route cache database.
    // Note: the loader process is writing to this; we should make sure we are
    // only reading from it.
    this.db = JSDB.open(path.join(this.basePath, '.cache'))
        ...
        filesOfType.forEach(async filePath => {
          ...
          const route = filePath
            ...
            if (handlerRaw.render) {
              ...
              // TODO: BROKEN: OK, so this isn’t going to work because the loader process is
              // writing to the JSDB database but we don’t get alerted to the changes (remember
              // JSDB was designed for a single process.)
              //
              // It only works in the spike because I’m opening the database _after_ I import
              // the route.
              //
              // There are ways to work around this:
              //
              //   - Instead of JSDB, just write the cache to disk and read it in from this process.
              //   - Implement a read-only watch mode for JSDB where you get streaming updates
              //     from the database whenever it changes.
              //
              // LEFT OFF HERE.
              //
              console.log('[FILES] Attempting to get route cache for route', route)
              const routeCache = this.db.routes[route]
