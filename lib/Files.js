...
export default class Files {
  ...
  async createRoutes () {
    ...
    for await (const extension of extensions) {
      ...
      for await (const filePath of filesOfType) {
        ...
        if (extension === 'socket') {
          ...
          console.log('[FILES]', webSocketHandler.toString())
          ...
          console.log('[FILES] Adding route', httpMethod, route, filePath, handler.toString())
    ...
    console.log('!!!!!!!!!!!', this.app.routes.forEach(route => {
      console.log('route', route, route.handlers)
    }))

    console.log('Admin index route info')
    const adminIndexRoute = this.app.find('get', '/admin/index/something')
    console.log('>>>>>>>>>>>', adminIndexRoute, adminIndexRoute.handlers[0].toString(), '============================================', adminIndexRoute.handlers[1].toString())
